...
def render_combined():
    ...
    try:
        ...
        st.success(
            f"Data digabungkan: {len(df_west_sel)} baris dari West, {len(df_east_sel)} baris dari East, total {len(df_combined)} baris."
        )
        st.markdown("---")
...
# Tabs (replaces radio)
# -------------------------
tab_west, tab_east, tab_combined = st.tabs(["West", "East", "Combined"])
with tab_west:
    render_west()
with tab_east:
    render_east()
